...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/btnproject/btn>";
    const std::string URL_WEBSITE = "<https://btn.org>";
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                if (fReindex) {
                    boost::filesystem::path stateDir = GetDataDir() / "stateBtn";
                ...
                boost::filesystem::path btnStateDir = GetDataDir() / "stateBtn";

                bool fStatus = boost::filesystem::exists(btnStateDir);
                const std::string dirBtn(btnStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsBtnstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<BtnState>(new BtnState(dev::u256(0), BtnState::openDB(dirBtn, hashDB, dev::WithExisting::Trust), dirBtn, existsBtnstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::btnMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult = new StorageResults(btnStateDir.string());
