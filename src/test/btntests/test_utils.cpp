...
void initState(){
    ...
    const std::string dirBtn = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<BtnState>(new BtnState(dev::u256(0), BtnState::openDB(dirBtn, hashDB, dev::WithExisting::Trust), dirBtn + "/btnDB", dev::eth::BaseState::Empty));
...
BtnTransaction createBtnTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    BtnTransaction txEth;
    if(recipient == dev::Address()){
        txEth = BtnTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = BtnTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
