...
void checkResult(bool isCreation, std::vector<BtnTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    BtnTxConverter converter(transaction, NULL);
    ExtractBtnTX btnTx;
    BOOST_CHECK(converter.extractionBtnTransactions(btnTx));
    std::vector<BtnTransaction> result = btnTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(btntxconverter_tests, TestingSetup)
