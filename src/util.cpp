...
const char * const BITCOIN_CONF_FILENAME = "btn.conf";
const char * const BITCOIN_PID_FILENAME = "btnd.pid";
...
void OpenDebugLog()
{
    ...
    assert(fileout == NULL);
    assert(fileoutVM == NULL); // btn
    assert(vMsgsBeforeOpenLog);
    boost::filesystem::path pathDebug = GetDataDir() / "debug.log";
    boost::filesystem::path pathDebugVM = GetDataDir() / "vm.log"; // btn
    fileout = fopen(pathDebug.string().c_str(), "a");
    fileoutVM = fopen(pathDebugVM.string().c_str(), "a"); // btn
    if (fileout) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Btn
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Btn
    // Mac: ~/Library/Application Support/Btn
    // Unix: ~/.btn
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Btn";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Btn";
#else
    // Unix
    return pathRet / ".btn";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("BTN Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The BTN Core developers";
    }
