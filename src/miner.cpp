...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setBtnSchedule(btnDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = btnDGP.getBlockSize(nHeight);
    minGasPrice = btnDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = btnDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    BtnTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractBtnTX resultConverter;
    if(!convert.extractionBtnTransactions(resultConverter)){
    ...
    for(BtnTransaction btnTransaction : btnTransactions){
        txGas += btnTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + btnTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(btnTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, btnTransactions, hardBlockGasLimit);
...
void StakeBtns(bool fStake, CWallet *pwallet)
{
