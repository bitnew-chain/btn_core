...
bool ToHash160(const std::string& strBtnAddress, std::string& strHash160)
{
    CBitcoinAddress btnAddress(strBtnAddress);
    if(btnAddress.IsValid()){
        CKeyID keyid;
        btnAddress.GetKeyID(keyid);
...
bool ToBtnAddress(const std::string& strHash160, std::string& strBtnAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CBitcoinAddress btnAddress;
    btnAddress.Set(keyid);
    if(btnAddress.IsValid()){
        strBtnAddress = btnAddress.ToString();
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToBtnAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToBtnAddress(tokenEvent.receiver, tokenEvent.receiver);
