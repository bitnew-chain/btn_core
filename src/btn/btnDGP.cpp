...
bool BtnDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule BtnDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t BtnDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t BtnDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t BtnDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t BtnDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool BtnDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void BtnDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void BtnDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void BtnDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void BtnDGP::createParamsInstance(){
    ...
    if(storageDGP.count(paramsInstanceHash)){
        ...
        for(size_t i = 0; i < size_t(paramsInstanceSize); i++){
            ...
            params.first = toUint64(storageDGP.find(sha3(paramsInstanceHash))->second.second);
            ++paramsInstanceHash;
...
dev::Address BtnDGP::getAddressForBlock(unsigned int blockHeight){
...
void BtnDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
    ...
    for(std::pair<dev::u256, dev::u256> d : data){
        ...
        for(size_t i = 0; i < 4; i++){
            uint64_t uint64Value = toUint64(value);
            value = value >> 64;
...
void BtnDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
    ...
    for(size_t i = 0; i < size; i++){
        ...
        uint32Values.push_back(toUint64(dev::u256(valueTemp)));
    }
}

void BtnDGP::parseStorageOneUint64(uint64_t& value){
    dev::h256 blockSizeHash = sha3(dev::h256(dev::u256(0)));
    if(storageTemplate.count(blockSizeHash)){
        value = toUint64(storageTemplate.find(blockSizeHash)->second.second);
    }
}

void BtnDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule BtnDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
