...
BtnState::BtnState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = BtnState::openDB(_path + "/btnDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

BtnState::BtnState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute BtnState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, BtnTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createBtnAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            btn::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> BtnState::vins() const // temp
{
...
void BtnState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* BtnState::vin(dev::Address const& _a) const
{
    return const_cast<BtnState*>(this)->vin(_a);
}

Vin* BtnState::vin(dev::Address const& _addr)
{
...
// void BtnState::commit(CommitBehaviour _commitBehaviour)
// {
...
void BtnState::kill(dev::Address _addr)
{
...
void BtnState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address BtnState::createBtnAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void BtnState::deleteAccounts(std::set<dev::Address>& addrs){
...
void BtnState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void BtnState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
