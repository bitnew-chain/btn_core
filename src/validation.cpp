...
#if defined(NDEBUG)
# error "Btn cannot be compiled without assertions."
#endif
...
std::unique_ptr<BtnState> globalState;
...
const std::string strMessageMagic = "Btn Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // btn
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            BtnDGP btnDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = btnDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = btnDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            BtnTxConverter converter(tx, NULL);
            ExtractBtnTX resultConverter;
            if(!converter.extractionBtnTransactions(resultConverter)){
            ...
            std::vector<BtnTransaction> btnTransactions = resultConverter.first;
            std::vector<EthTransactionParams> btnETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(BtnTransaction btnTransaction : btnTransactions){
                sumGas += btnTransaction.gas() * btnTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, btnTransaction.gasPrice());
                } else {
                    txMinGasPrice = btnTransaction.gasPrice();
                }
                VersionVM v = btnTransaction.getVersion();
                if(v.format!=0)
                ...
                if(btnTransaction.gas() < GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(btnTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(btnTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)btnTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(btnETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > btnTransactions.size())
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s BTN additional fees, %d delta bytes\n",
...
static DisconnectResult DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // btn
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // btn
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    BtnTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<BtnTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path btnDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(btnDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(btnDir.string(), std::ios::in | std::ios::out);
...
bool BtnTxConverter::extractionBtnTransactions(ExtractBtnTX& btntx){
    std::vector<BtnTransaction> resultTX;
    ...
    btntx = std::make_pair(resultTX, resultETP);
    return true;
}

bool BtnTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool BtnTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
BtnTransaction BtnTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    BtnTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = BtnTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = BtnTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // btn
    BtnDGP btnDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setBtnSchedule(btnDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = btnDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = btnDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = btnDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // btn
...
///////////////////////////////////////////////////////////////////////////////////////// btn
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            BtnTxConverter convert(tx, &view, &block.vtx);

            ExtractBtnTX resultConvertBtnTX;
            if(!convert.extractionBtnTransactions(resultConvertBtnTX)){
            ...
            if(!CheckMinGasPrice(resultConvertBtnTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertBtnTX.first, blockGasLimit);
            ...
            for(BtnTransaction& qtx : resultConvertBtnTX.first){
            ...
            if (fLogEvents)
            {
                for(size_t k = 0; k < resultConvertBtnTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertBtnTX.first[k].from(), resultConvertBtnTX.first[k].to(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().GetConsensus().fPoSNoRetargeting) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(560000000 + ((pindex->nHeight - 5000) * 5)) * COIN){
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // btn
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // btn
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // btn
            globalState->setRootUTXO(oldHashUTXORoot); // btn
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // btn
        uint32_t sizeBlockDGP = btnDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Btn PoS we need to sync this after every block to ensure txdb is populated for
