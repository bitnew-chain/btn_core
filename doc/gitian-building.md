...
Clone the git repositories for btn and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/btnproject/btn --recursive
git clone https://github.com/btnproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/btn/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the btn repository under 'Fetch and create inputs' to install sources which require
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/btn/.git/
...
For example:
```bash
URL=https://github.com/laanwj/btn.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit btn=${COMMIT} --url btn=${URL} ../btn/contrib/gitian-descriptors/gitian-linux.yml
...
and the btn git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../btn/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/btnproject/btn-detached-sigs.git

BTCPATH=/some/root/path/btn
SIGPATH=/some/root/path/btn-detached-sigs

./bin/gbuild --url btn=${BTCPATH},signature=${SIGPATH} ../btn/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/btn-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/btn-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/btn-osx-build.assert
```
...
[btnproject/gitian.sigs](https://github.com/btnproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@btn.org) and he will commit them.
