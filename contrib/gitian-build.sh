...
Run this script from the directory containing the btn, gitian-builder, gitian.sigs, and btn-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/btnproject/btn
...
then
	...
	make -C ../btn/depends download SOURCES_PATH=`pwd`/cache/common
...
then
	...
	echo ""
	./bin/gverify -v -d ../gitian.sigs/ -r ${VERSION}-osx-signed ../btn/contrib/gitian-descriptors/gitian-osx-signer.yml
	# Signed Mac OSX
	echo ""
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../btn/contrib/gitian-descriptors/gitian-win-signer.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-win-signed --destination ../gitian.sigs/ ../btn/contrib/gitian-descriptors/gitian-win-signer.yml
	    mv build/out/btn-*win64-setup.exe ../btn-binaries/${VERSION}
	    mv build/out/btn-*win32-setup.exe ../btn-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=master ../btn/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ./bin/gsign --signer $SIGNER --release ${VERSION}-osx-signed --destination ../gitian.sigs/ ../btn/contrib/gitian-descriptors/gitian-osx-signer.yml
	    mv build/out/btn-osx-signed.dmg ../btn-binaries/${VERSION}/btn-${VERSION}-osx.dmg
	fi
	popd
